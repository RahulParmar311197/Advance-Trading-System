...
- [x] Deployment — provider-neutral production Compose topology, fail-closed production configuration validation, service-specific database/Redis URL scheme validation, health-gated startup, API readiness healthcheck for PostgreSQL/Redis/queue dependencies, CI Compose validation, and deployment/rollback runbook; no cloud provider is claimed
...
- [x] Readiness dependency failure hardening — PostgreSQL connection failures and Redis client-construction failures return structured unhealthy readiness responses; corrected fixture alignment passed Python/Compose CI on run `34588714810` / job recorded by the workflow.
...
- [x] Reproducible experiment runner — executes an injected deterministic experiment against its complete manifest, persists the manifest before results, supports rerunning from the stored manifest, validates executor result shapes with unit coverage, and the API rerun endpoint now delegates execution/persistence to the runner. The integration revision passed Python/Compose CI on run `34593278404` / job `103243201697.
