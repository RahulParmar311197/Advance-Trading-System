...
## Verification
- Python CI run `34316509982` completed successfully after the latest research/report implementation.
...
- New experiment comparison/report API and dashboard integration have unit coverage; CI verification is pending for these latest commits.
...
4. Verify walk-forward, out-of-sample, stress-test, strategy-comparison, and report implementations in CI.
5. Continue P1 portfolio/accounting dependencies required by the architecture before advanced market intelligence.

## Next dependency
After CI verification, complete any remaining P1 portfolio/accounting dependencies (`packages/portfolio/exposure.py`, `packages/portfolio/correlation.py`, `packages/backtest/broker_simulator.py`, and `packages/backtest/portfolio.py`) before moving to P2 options, microstructure, and regime work. Real provider configuration and full-stack acceptance remain explicit blockers.
