...
- P2 options foundation: validated immutable option-contract/chain model, dependency-free European Black-Scholes Greeks, deterministic Black-Scholes price/inversion IV, deterministic OI/OI-change analytics, and deterministic put/call open-interest ratio analytics with unit tests.
...
## Remaining blockers
1. Implement and CI-verify the remaining P2 options analytics: term structure, volatility surface, and payoff analysis.
...
## Next dependency
Implement options term-structure analytics from supplied option-chain observations and implied-volatility observations only. Do not fabricate option-chain data.
